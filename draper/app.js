...
const steps=[['Understand your business.','We find the expertise, convictions and customer problems that give your company something worth saying.'],['Build your presence.','We turn those ideas into a consistent voice across your leadership, employees and company page. You approve the work. We run the channel.'],['Turn attention into opportunity.','We track the conversations and commercial signals the work creates, then refine the narrative around what matters to your business.']];let step=0;
const selectStep=wireTabs('[data-step]',(i,t)=>{step=i;$('#step-title').textContent=steps[i][0];$('#step-copy').textContent=steps[i][1];document.querySelectorAll('[data-specimen]').forEach((el,n)=>el.hidden=n!==i);$('#process-panel').setAttribute('aria-labelledby',t.id)});
$('#step-next').addEventListener('click',()=>selectStep((step+1)%3));$('#step-prev').addEventListener('click',()=>selectStep((step+2)%3));
const arcStories=[
